	...
	return (
		...
		>
			<FieldGroup className="gap-5 overflow-y-auto px-4 py-1 md:-mx-2 md:px-2">
							...
							{(["expense", "income"] as const).map((dir) => {
								...
								return (
									<button
										...
										type="button"
										onClick={() => {
											field.handleChange(dir);
											// Q9: the category picker filters by direction — drop a selection
											// that no longer matches rather than leave a stale cross-kind pick.
				...
				{/* Category — filtered to the selected direction's kind (Q9) */}
				<form.Field
					name="categoryId"
					children={(field) => (
						<Field>
							<FieldLabel htmlFor={`${idPrefix}-category`}>Category</FieldLabel>
							<form.Subscribe selector={(s) => s.values.direction}>
								{(direction) => (
									<CategoryCombobox
										...
										onCreate={(input) =>
											onCreateCategory({ ...input, kind: direction })
										}
										allowNone
