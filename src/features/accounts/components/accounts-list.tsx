...
function netWorthByCurrency(accounts: Account[]): [string, number][] {
  const totals = new Map<string, number>()
  for (const a of accounts) {
    if (a.closedAt || a.excludeFromNetWorth) continue
    totals.set(a.currency, (totals.get(a.currency) ?? 0) + a.initialBalance)
  }
...
function AccountRow({ account }: { account: Account }) {
  ...
  return (
    <Link
      ...
      className={cn(
        "flex items-center gap-3 px-4 py-3.5 transition-colors hover:bg-muted/40 focus-visible:bg-muted/40 focus-visible:outline-none",
        closed && "opacity-60",
      )}
    >
      <span
...
function AccountCard({ account }: { account: Account }) {
  ...
  return (
    <Link
      ...
      params={{ accountId: account.id }}
      className={cn(
        "flex flex-col gap-4 rounded-xl border bg-card p-4 transition-colors hover:bg-muted/40 focus-visible:bg-muted/40 focus-visible:outline-none",
        closed && "opacity-60",
      )}
    >
...
export function AccountsList({ accounts }: { accounts: Account[] }) {
  const totals = netWorthByCurrency(accounts)
