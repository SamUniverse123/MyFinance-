...
/* ────────────────────────────────────────────────────────────────────────────
   Better-auth tables (generated by better-auth's Drizzle adapter)
   Import from your generated auth schema instead of redefining — shown here only
   so the FKs below resolve. NOTE: user.id is `text`, not uuid.
   ──────────────────────────────────────────────────────────────────────────── */
...
export const transactionSplits = pgTable('transaction_splits', {
  ...
  categoryId: uuid('category_id').references(() => categories.id),
  amount: bigint('amount', { mode: 'number' }).notNull(), // SUM must equal parent.amount (enforce in app)
  note: text('note'),
}, (t) => [
