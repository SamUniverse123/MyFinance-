...
export const Route = createFileRoute('/_app/transactions/$transactionId')({
  loader: ({ context, params }) =>
    Promise.all([
      context.queryClient.ensureQueryData(transactionDetailOptions(params.transactionId)),
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-xl border bg-card p-5 md:p-6">
        ...
        <p className="mt-1 text-xs text-muted-foreground">{income ? 'Money in' : 'Money out'}</p>
      </section>

      <section className="rounded-xl border bg-card p-5 md:p-6">
        <h3 className="mb-4 text-sm font-medium">Details</h3>
        <dl className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3">
          <DetailItem label="Account" value={account?.name ?? 'Unknown account'} />
          <DetailItem label="Date" value={fmtDay(transaction.date)} />
          <DetailItem label="Status" value={<span className="capitalize">{transaction.status}</span>} />
          <DetailItem label="Currency" value={transaction.currency} />
          {transaction.payeeName && <DetailItem label="Payee" value={transaction.payeeName} />}
          <DetailItem label="Created" value={fmtTimestamp(transaction.createdAt)} />
          <DetailItem label="Updated" value={fmtTimestamp(transaction.updatedAt)} />
        </dl>
        {transaction.note && (
          <>
            <Separator className="my-5" />
            <div className="flex flex-col gap-0.5">
              ...
              <dd className="text-sm whitespace-pre-wrap">{transaction.note}</dd>
            </div>
          </>
        )}
      </section>
...
function TransactionDetailPage() {
  ...
  const { data: transaction, isPending, isError, error, refetch } = useGetTransaction(transactionId)
        ...
        <div className="flex flex-1 items-center justify-center py-10">
          <Empty>
            <EmptyHeader>
              ...
              <EmptyTitle>{notFound ? 'Transaction not found' : "Couldn't load this transaction"}</EmptyTitle>
              <EmptyDescription>
                {notFound
